...
void refineBiasPositive(double *mPostB, double *vPostB, double vPriorB,
	...
	int *nOnesSampling, int *nZerosSampling) {
	...
	eta2New = eta2Prior - 2 * (nOnesSampling[ T - 1 ] +
		nZerosSampling[ T - 1 ]) * lambda(zeta);

	eta1New = (nOnesSampling[ T - 1 ] + nZerosSampling[ T - 1 ]) *
		(0.5 + 2 * lambda(zeta) * mPred) + eta1Prior;
...
void refineBiasNegative(double *mPostB, double *vPostB, double vPriorB,
	double mPriorB, double mPred, double vPred, int T, double rho,
	int *nZerosSampling, int *nOnesSampling, double *e0) {
	...
	eta1New = (nZerosSampling[ T - 1 ] + nOnesSampling[ T - 1 ]) *
		(-(1 - 2 * e) / 2 + 2 * lambda(zeta) * mPred) + eta1Prior;
...
void refineRowUPositive(double *mPostU, double *vPostU, double *mPostV,
	...
	for (h = 0 ; h < k ; h++) {
		...
		*vPred -= mPostU[ i + h * T ] * mPostU[ i + h * T ] *
		...
		reps = (double) nZerosAuxi[ i ] + nOnesAuxi[ i ];
